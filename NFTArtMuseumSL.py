...
def main_routine():
    ...
    page = st.sidebar.selectbox("Choose ", ["Register Art", "Buy NFT"])
    st.title('Welcome to NFT Art Museum')
    if page == "Register Art":
        st.header("NFT Art Registry page.")
        ...
        price = st.slider("Select the price of NFT in Wei", 10000, 50000000)
        st.text('Selected: {}'.format(price))
        artist_private_key = st.text_input("Please enter your private key: ", key=  "artist_private_key", type="password")
        ...
        if st.button('Register Art'):
            ...
            if error_sign != 1 and error_send != 1:
                st.write('Art registry successful!! congrats. Your txn = ', register_signed_txn_hash)
